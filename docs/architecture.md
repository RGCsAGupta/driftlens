...
One application instance runs at most one active scan. A second start request
receives a structured conflict response. This bounds resource use and keeps
Core behavior explicit.
...
continues the scan while the UI polls. Graceful shutdown allows a bounded
10–30 second drain, but Core does not resume unfinished work after termination.
...
explanation representation. Active-scan admission is atomic in SQLite rather
than protected only by an in-memory flag.
...
- DriftLens remains outside Kubernetes, initially on pve1 unless placement is
  changed through an approved decision.
...
- Cloudflare-protected public access is an Extended step after local Core
  acceptance; direct origin access must not bypass it.

The exact container registry, process supervisor, domain, and Cloudflare
configuration belong to the delivery specification.
...
relies on operator-controlled network placement; hosted access relies on
Cloudflare protection in Extended scope.
...
| One active scan | Bounded, transparent behavior | No concurrent scans |
...
Cloudflare hosting, multiple clusters, scheduling, alerts, remediation,
authentication, audit retention, and high availability.
...
At the architecture review checkpoint (`2026-07-30T20:21:24Z`), recorded
assessment work is 61 minutes 53 seconds. The provisional remaining-work
budget below totals 178 minutes and must be recalculated from the live ledger
before the first implementation issue begins:

| Slice | Target |
| --- | ---: |
| Foundation | 18 minutes |
| Deterministic API and persistence | 52 minutes |
| Operator UI | 27 minutes |
| AI explanation | 14 minutes |
| Demo, deployment, docs, and evidence | 42 minutes |
| Risk reserve | 25 minutes |

Cloudflare, private Git, configuration CRUD, resource quantities, and hosted
public access do not begin inside this Core budget.
